        ...
        <ul className="grid grid-cols-2 md:grid-cols-4 col-span-12 sm:col-span-9 ml-0 sm:ml-20 order-1 md:order-5">
          <li className="mt-2 text-left">
            ...
            <ul>
              ...
              <li className="my-1 sm:my-4">
                <a href="https://satlaa.com/blogs" className="text-black">Blogs</a>
              </li>
              <li className="my-1 sm:my-4">
                <Link href="/warranty" className="text-black">Warranty</Link>
              </li>
              <li className="my-1 sm:my-4">
                <Link href="/contact-us" className="text-black">Contact Us</Link>
              </li>
              <li className="my-1 sm:my-4">
                <Link href="/pages/certificates" className="text-black">Certificates</Link>
              </li>
            </ul>
          </li>
          {footerMenu?.map((val) => (
            <li key={val.title} className="mt-2 text-left">
              {!val.children ? (
                <Link href={val.link || `/pages/${val.seo}`} className="text-black text-2xl">
                  {val.title}
                </Link>
              ) : (
                <>
                  ...
                  <ul>
                    {val.children.map((val2) => (
                      <li key={val2.title} className="my-1 sm:my-4">
                        {!val2.children ? (
                          <Link href={val2.link || `/pages/${val2.seo}`} className="text-black">
                            {val2.title}
                          </Link>
                        ) : (
                          <>
                            <Link href="#" className="text-black">-{val2.title}</Link>
                            <ul>
                              {val2.children.map((val3) => (
                                <li key={val3.title}>
                                  <Link href={val3.link || `/pages/${val3.seo}`} className="text-black">
                                    --{val3.title}
                                  </Link>
                                </li>
                              ))}
        ...
        <div className="col-span-12 text-center order-10">
          <div className="text-black grid grid-cols-2 md:flex items-center flex-row justify-around md:grid-cols-3 lg:grid-cols-6">
            ...
            {settings.phone?.map((val) => (
              <div key={val.name}>
                <h5 className="text-xl text-black mt-2">{val.name}</h5>
                <a className="underline" href={`tel:${val.value}`}>{val.value}</a>
              </div>
            ))}
            {settings.email?.map((val) => (
              <div key={val.name}>
                <h5 className="text-xl text-black mt-2">{val.name}</h5>
                <a href={`mailto:${val.value}`} className="underline">{val.value}</a>
              </div>
            ))}
          </div>
        </div>
